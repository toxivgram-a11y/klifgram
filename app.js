...
$('#withdraw-button').addEventListener('click', () => { const amount = Number($('#withdraw-amount').value); if (!amount || amount < 100) return toast('Минимальная сумма вывода: 100 кредитов'); if (amount > state.balance) return toast('Недостаточно кредитов'); if (!$('#withdraw-destination').value.trim()) return toast('Укажи кошелёк или username'); toast('Заявка на вывод создана'); });
...
function openGame(game) { modal.classList.remove('hidden'); $('#modal-tag').textContent = game.toUpperCase(); $('#modal-title').textContent = { mines: 'Мины', crash: 'Краш', upgrade: 'Апгрейд' }[game]; $('#modal-subtitle').textContent = { mines: 'Выбери безопасную клетку', crash: 'Забери выигрыш до падения', upgrade: 'Попробуй улучшить предмет' }[game]; const content = $('#game-content'); if (game === 'mines') { content.innerHTML = `<div class="game-controls"><input id="mines-bet" type="number" min="10" value="50"><button id="mines-start">Поставить</button></div><div class="mines-board">${Array.from({ length: 15 }, (_, i) => `<button class="mine-cell" data-cell="${i}">?</button>`).join('')}</div>`; $('#mines-start').onclick = () => { const bet = Number($('#mines-bet').value); if (bet < 10 || bet > state.balance) return toast('Проверь размер ставки'); state.balance -= bet; document.querySelectorAll('.mine-cell').forEach(cell => cell.onclick = () => { const safe = Math.random() > .25; cell.textContent = safe ? '◆' : '×'; cell.style.background = safe ? '#bdeac0' : '#ffb29d'; cell.disabled = true; if (!safe) { addGameResult('Мины', -bet); toast('Мина! Ставка потеряна'); } else { const prize = Math.round(bet * 1.35); addGameResult('Мины', prize); toast(`Безопасно! +${prize} TC`); } }); save(); toast('Сделай ход'); }; } else if (game === 'crash') { content.innerHTML = `<div class="crash-screen"><div class="multiplier" id="multiplier">1.00x</div><p class="muted">Ставка: 50 TC</p><button id="crash-start">Запустить раунд</button></div>`; let timer; $('#crash-start').onclick = () => { const bet = 50; if (state.balance < bet) return toast('Недостаточно TC'); state.balance -= bet; let mult = 1; $('#crash-start').disabled = true; timer = setInterval(() => { mult += .11; $('#multiplier').textContent = `${mult.toFixed(2)}x`; if (Math.random() < .08) { clearInterval(timer); addGameResult('Краш', -bet); $('#multiplier').textContent = `${mult.toFixed(2)}x`; $('#crash-start').textContent = 'Раунд завершён'; toast('Краш!'); } }, 150); }; } else { content.innerHTML = `<div class="upgrade-visual">◇<p class="muted">Шанс успеха: 52%</p><button id="upgrade-start">Апгрейд за 75 TC</button></div>`; $('#upgrade-start').onclick = () => { if (state.balance < 75) return toast('Недостаточно TC'); state.balance -= 75; const win = Math.random() > .48; addGameResult('Апгрейд', win ? 130 : -75); toast(win ? 'Апгрейд успешен! +130 TC' : 'Апгрейд не удался'); closeModal(); }; } }
